...
class AnalysisProcessor:
    ...
    async def process(self, job: AnalysisJob) -> str:
        ...
        latest_search_rows: LatestProductLocationSelector[ClassifiedOffer] = (
            LatestProductLocationSelector()
        )
        ...
        distribution_search_counts: dict[str, int] = defaultdict(int)
        distribution_stores: dict[str, set[str]] = defaultdict(set)
        service_area_scopes: dict[str, set[str]] = defaultdict(set)
        ...
        async def consume(row: dict[str, Any]) -> None:
            ...
            try:
                ...
                # Conflicting or malformed boolean aliases indicate source-schema
                # drift. Preserve the fail-closed normalization boundary even though
                # stock and sponsorship do not gate price or distribution.
                raise
            except ValueError:
                return
            ...
            if is_product_location_state(classified_offer):
                add_classified_offer(latest_search_rows, classified_offer)
        ...
        for classified_offer in latest_search_rows.values():
            normalized_offer = classified_offer.offer
            retailer_id = normalized_offer.retailer_id
            if (
                is_seller_policy_exclusion(classified_offer)
                or normalized_offer.price is None
                or normalized_offer.price <= 0
            ):
                continue
            distribution_store_id = str(normalized_offer.store_number or "").strip()
            if distribution_store_id:
                distribution_search_counts[retailer_id] += 1
                distribution_stores[retailer_id].add(distribution_store_id)
            elif normalized_offer.zipcode is not None:
                service_area_scopes[retailer_id].add(normalized_offer.zipcode)
        ...
        coverage = [
            {
                ...
                "distribution_search_offers": distribution_search_counts[retailer_id],
                "distribution_stores": len(distribution_stores[retailer_id]),
                "service_area_presence_count": len(service_area_scopes[retailer_id]),
