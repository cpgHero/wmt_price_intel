      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            [
              "2026-08-30",
              "Implemented and test-verified; migration, production registration, and any paid recovery pending",
              ...
              "Migration 0051 adds an offline immutable spend authorization consumed by one recovery batch, exact and legacy-adoption plans, retailer-unavailability approvals, canonical task selections, component manifests, and one selected lineage row per provider request. Materialization reads the governed retailer-location eligibility field and explicitly types lineage values before PostgreSQL JSONB construction, preventing stale scopes or driver-dependent manifests. CI rehearses upgrade, downgrade, and upgrade on a pristine database before integration tests create protected audit lineage; the production downgrade remains fail-closed once that lineage exists. The exact seven-run Banana/Milk/Egg inventory totals 67,908 actual credits ($135.816) and must be registered under the owner's 100,000-credit ceiling ($200 at the server-controlled $0.002/credit). The HTTP API accepts only the authorization UUID, so it cannot invent a phase, inventory, rate, or cap. Exact reservations use credits-per-call times maximum attempts, forbid unapproved pagination descendants and overlapping active selections, preserve successful and billable-404 evidence, and launch idempotently without a gate. Legacy adopted base/recovery runs must both be in the same authorized inventory. Raw collection readiness is separate from report semantics; explicit unavailable competitors remain visible in source/readiness but are excluded from every scorecard and assortment computation. Contract/quarantine evidence still blocks. The frozen Egg Kroger scope includes 1,298 now-ineligible aliases, so paid recovery is blocked and Kroger is unavailable pending a Phase-0052 audit-bound 1,369-row projection. Meijer is likewise unavailable rather than a zero scorecard. Ready composites queue one analysis; blocked composites queue none. No provider call, deployment, production batch registration, or source-data mutation was made.",
            ],
            [
              "2026-08-30",
