      ...
      blocks: [
        {
          ...
          text: "First, preserve each Search-listed price together with its retailer product and store- or service-area query context. Second, count an exact product's observed store distribution from distinct store-level Search results with price greater than $0; report service-area presence separately. Do not extrapolate either measure to locations that were not observed. Every summary must remain traceable to those atomic observations.",
        },
        {
          kind: "definitions",
          ...
          items: [
            {
              ...
              definition:
                "Examines a retailer's own products, Search-listed prices, observed store distribution, separately labeled service-area presence, sponsorship, and query-context price exceptions. Store distribution counts each distinct store once when the exact product appears in a store-level Search result with price greater than $0. It is not an inventory or in-stock measure and is never extrapolated.",
            },
            {
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            [
              "Price Intelligence",
              "One-retailer product, Search-listed price, observed store distribution, separate service-area presence, architecture, review, and future history.",
              "Distribution is the distinct positive-price store-level Search footprint. It is not inventory or in-stock status, is not extrapolated, and keeps service areas separate from stores.",
            ],
            [
            ...
            [
              ...
              "Uses eligible location-master coordinates only. It is straight-line distance, not drive time, product assortment, inventory, or observed product distribution. Walmart-side metrics must disclose whether their denominator is all Walmart locations in the selected country or only Walmart locations in the selected competitor's sourced state/province footprint.",
            ],
            [
              "Collections",
      ...
      blocks: [
        {
          ...
          items: [
            ...
            },
            {
              ...
              detail:
                "For each retailer × product × location, select the latest Search evidence, preserve its source fields for audit, and enrich geography from the location master. An in-scope store-level row with price greater than $0 enters observed store distribution; deduplicate by retailer product ID × store ID. Service-area rows enter a separate service-area presence count. Stock and sponsorship flags do not determine either footprint, and no count is extrapolated to unobserved locations.",
            },
            {
            ...
            },
            {
              ...
              detail:
                "High-recall candidates are blocked by known hard conflicts, evaluated against Product Pack tiers, and scoped to positive-price observed-distribution overlap. Price similarity is never a semantic matching signal.",
              link: {
      ...
      blocks: [
        {
          kind: "callout",
          tone: "attention",
          title: "Observed distribution release boundary",
          text: "The current report contracts define observed store distribution as distinct stores where the exact retailer product ID appears in a store-level Search result with price greater than $0. Stock flags and sponsorship do not determine this footprint. Service-area presence is a separate count and is never relabeled as stores. Reprocessing retained evidence creates corrected immutable reports without repeating provider collection.",
        },
        {
          kind: "definitions",
          ...
          items: [
            {
              term: "Price Intelligence",
              definition:
                "Aggregates canonical product-location evidence without requiring product matches. Home selects an exact retailer product. The cross-retailer Price Architecture Matrix places every positive-price SKU into Walmart-defined or fixed Search-price bands. Product Overview combines identity, observed store distribution, separate service-area presence, price, sponsorship, and map evidence; Product Price Architecture explains one SKU's positive Search-price distribution; Store Review focuses unusual Search-listed prices and Search non-observations.",
            },
            {
              term: "Competitive Intelligence",
              definition:
            ...
            },
            {
              term: "Proximity",
              definition:
                "Explores location-master store-network distance before a collection or report. The current app page fixes Walmart US or Walmart CA as the benchmark, requires exactly one selected competitor retailer, computes the nearest selected competitor location for every mappable Walmart location, and exposes the same filtered rows through app-shell controls, a map-first workspace, filter/detail drawers, table drawer, CSV, Excel-compatible CSV, JSON, and GeoJSON downloads. The API returns backend-prepared map clusters plus distance, state-coverage, and competitor-network summaries; filtered UI views recompute those same measures from the active row set. Radius changes preserve the selected competitor retailer. Walmart-side coverage and white-space metrics expose an explicit comparison scope: all Walmart locations in the selected country, or only Walmart locations in states/provinces where the selected competitor has mappable location-master rows. The competitor-footprint scope is recommended for clearly regional competitors such as H-E-B and is state/province based only; it is not a DMA, CBSA, drive-time, trade-area, assortment, inventory, Search-observation, or sales-opportunity claim. Counts come from eligible location-master rows with coordinates.",
            },
          ],
        },
        {
          ...
          items: [
            ...
            "Name every denominator: observed-distribution stores, searched stores, service-area Search contexts, matched observations, scored benchmark stores, or another explicit governed population.",
            "Observed store distribution counts each distinct store once when an exact retailer product ID appears in that store-level Search result with price greater than $0. It does not use store-level stock status, does not claim current inventory, and never extrapolates to unobserved stores. Service-area presence is calculated and labeled separately.",
            "Comparable Store Coverage is the share of distinct benchmark stores with positive-price Search evidence and at least one valid local competitor comparison under the selected retailer, comparison basis, period, geography, and radius. Count each benchmark store once even when several products contribute evidence; count each contributing competitor store once. Service-area retailers use distinct delivery ZIPs instead of stores.",
            ...
            "Assign every unscored local context a reason such as no eligible match, no overlap, no positive-price Search evidence, stale or missing price, collection failure, incomplete attributes, or review required.",
            ...
            "For physical retailers, radius-native scorecards rebuild certified product relationships at product × observed-distribution Walmart store grain and require the competitor store to be within the selected 1, 3, or 5 mile radius. Service-area retailers remain explicitly same-delivery-ZIP because they do not expose a comparable physical store footprint.",
            ...
            "Proximity is a separate location-master analytics page, not a product report. It always uses Walmart US or Walmart CA against one selected competitor, computes nearest-store Haversine distances from eligible coordinates, keeps radius changes on the selected competitor, and keeps all radius, comparison-scope, state, search, table, map, and download views on the same filtered row set. For regional competitors, the competitor-footprint scope limits Walmart-side denominators to states/provinces where that competitor has sourced mappable locations; the all-Walmart scope remains available for explicit total-country exposure questions.",
            ...
            "The active analysis route renders the legacy Blueprint Competitive Intelligence report workspace. The newer canonical product-report workspace and simplified navigation are paused and are not imported by the live /analyses/[analysisId] page. Legacy Competitive Intelligence keeps its established retailer scorecard, cohort, footprint, matched matrix, match summary, price ladder, store comparison, history, and assortment views while Price Intelligence remains the product-level drilldown surface. Any future report redesign must keep complete Walmart-win and Walmart-loss action lists, source-backed filters, state-footprint context, product/store-evidence drawers, CSV, Excel-compatible, and JSON downloads, and explicit map-evidence availability. Price display must distinguish source-backed package price from normalized comparison value; a normalized $/gallon value is never labeled as shelf or package price unless source package-price evidence is present.",
            ...
            "Read Price Architecture Matrix rungs from the lowest Walmart price position to the highest. Exact canonical-brand filtering changes the displayed assortment but preserves the Walmart-defined rung boundaries. Every product card identifies the retailer product ID, observed store distribution or separately labeled service-area presence, and seller-governance state.",
            "Calculate matrix distribution coverage as the distinct union of positive-price store-level Search locations for any product in a cell. Never sum individual product footprints and never extrapolate to unobserved stores. An empty cell means no positive-price SKU was observed in that price band; it is not proof of retailer assortment absence.",
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "WorkOS AuthKit/User Management remains the selected future customer authentication provider, but the production web rollout is paused. Railway stores WorkOS variables and migrations 0055 and 0056 keep dormant account/workspace identity-mapping tables for a future controlled redesign. CPGHero remains the authorization source of truth for account/workspace membership, roles, permissions, entitlements, projects, report access, Live API usage limits, and billing. Enterprise SSO, Directory Sync, and SCIM are not part of the initial rollout.",
            "Migration 0056 adds the owner-only customer provisioning and identity-event audit foundation: prepared invitations, account/workspace user mappings, role and entitlement assignment, and a signed WorkOS webhook receiver. While the rollback is active, the public Next.js /api/webhooks/workos route returns a private no-store 204 acknowledgement and no longer forwards raw webhook payloads to the internal API receiver. Customer-facing responses remain CPGHero-facing and do not expose provider subject IDs.",
            "Customer-auth canary guardrails, invitation readiness, allowlists, and customer-session sealing remain historical rollout work and are not active in the live web application while customer authentication is paused. The Customer Auth administrator page redirects to System Operations rather than presenting a WorkOS management surface.",
            "The non-production customer-principal harness is explicit, opt-in, and header-backed for local/API tests only. Production rejects those headers. Production customer-login web flow is disabled during the restore, so live WorkOS customer sessions are not accepted by web routes.",
            "Legacy restore mode disables the Next.js route-boundary auth gate. The proxy performs no customer or administrator authentication validation; it returns NextResponse.next for non-customer page and API shells and hard-redirects dormant customer pages to legacy app destinations. routeAccessDecision classifies routes as public; web login, callback, and logout routes redirect to the legacy root; /api/auth/me returns private no-store JSON 401; /api/customer/reports routes return a private no-store disabled response without proxying; and /api/webhooks/workos acknowledges without forwarding. The root route is the legacy app home/dashboard, not a public landing page. Administrator pages and protected admin APIs use the existing legacy password session through /api/admin/session and verifyAdminAccess. API route handlers remain responsible for authoritative permission, entitlement, account, workspace, report-grant, and admin-operation enforcement.",
            "The prior Playwright route-auth bypass token is inactive while the proxy performs no auth validation; production must still not configure CPGHERO_WEB_ROUTE_AUTH_TEST_BYPASS_TOKEN.",
          ],
        },
        {
          kind: "table",
          ...
          rows: [
            [
              ...
              "Paused WorkOS foundation",
              "Web login, callback, and logout routes redirect to the legacy root; /api/auth/me returns private no-store JSON 401; /customer routes redirect out of the customer workspace; WorkOS external setup and API-side code remain dormant for a future redesign.",
            ],
            [
              "Identity mapping",
              "CPGHero",
              "Migrations 0055 and 0056 map WorkOS organization/user subjects to CPGHero accounts/app users, store prepared invitations, and audit signed identity events, but those rows are not consulted by live web routing or administrator access while customer auth is paused.",
            ],
            [
            ...
            [
              ...
              "CPGHero",
              "Protected Product Pack, matching, recovery, system operations, and report-publishing routes use the existing legacy administrator password session. WorkOS-backed customer principals do not grant Administration access during the restore.",
            ],
          ],
        },
        {
          ...
          text: "The durable account/RBAC/entitlement foundation exists, WorkOS AuthKit is externally configured, migration 0055 adds external identity mappings, and migration 0056 adds owner-only invitation/provisioning state plus signed identity webhook audit. The current production web application intentionally does not use that foundation for customer login, app routing, or administrator authorization. Customer-facing signup/invite screens, customer API keys, customer account administration, tenant-scoped report grants, and cross-account enforcement across legacy report, collection, proximity, and bulk-project data routes remain future work. Until that redesign is rebuilt and verified, the app uses the legacy global reporting/admin model and legacy administrator password session.",
        },
      ],
    },
    {
    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          text: "Search remains the listed package-price, query-context, sponsorship, and collection-time authority. Store distribution counts distinct store-level Search contexts where the exact product has price greater than $0; service-area presence remains separate. PDP enrichment runs only for distinct admitted analysis products, reuses evidence that is fresh under the current 30-day policy, and normally selects one representative positive-price Search location context per product. Another PDP context is justified only by contradictory identity evidence or a governed location/variant diagnostic. Paid-calls-enabled means the contract is eligible for planning—not that every product/location request will succeed.",
        },
        {
          kind: "table",
          ...
          rows: [
            ...
            [
              ...
              "Only Search may author listed price and observed distribution; price must be greater than $0",
            ],
            [
              "Search distribution",
              "Distinct product × store positive-price Search presence, plus separate service-area presence",
              "Stock status is retained only as raw audit evidence and is not used to calculate or label public distribution",
            ],
            [
              "Search location",
        ...
        },
        {
          ...
          rows: [
            [
              ...
              "Search positive listed price + exact product identity + frozen store ID",
              "Counts each observed store once; makes no inventory claim and never extrapolates to unobserved stores",
            ],
            [
              "Observed service-area presence",
              "Distinct service-area Search contexts returning the exact product with price > 0",
              "Search positive listed price + exact product identity + frozen service-area context",
              "Reported separately and never presented as a store count",
            ],
            [
              "Observed distribution coverage",
              "Observed distribution stores ÷ searched store scope",
              "Distinct positive-price store numerator + frozen searched-store denominator",
              "The numerator is never projected beyond observed stores; service-area contexts use their own separately labeled denominator",
            ],
            [
              "Not observed",
            ...
            [
              ...
              "Distinct exact-product locations with a Search-listed price greater than $0",
              "Distinct store or service-area query grain",
              "Physical stores become observed store distribution; service areas remain separately labeled presence",
            ],
            [
            ...
            [
              "Median Search-listed price",
              "Median of positive exact-product Search prices in the selected observed footprint",
              "Positive-price Search observations in the selected retailer/geography",
              "The UI must label package or normalized unit explicitly",
            ],
            [
              "Search-listed price range",
              "Minimum through maximum positive exact-product Search price",
              "Same positive-price rows as the Search-listed median",
              "The range describes observed Search prices, not inventory",
            ],
            [
              "Sponsored share",
            ...
            [
              ...
              "Exact-product positive Search-listed prices by query context + Product Pack tolerance",
        ...
        },
        {
          ...
          rows: [
            ...
            [
              ...
              "Eligible competitor product with a positive store-level Search price at the same delivery ZIP or physical store within selected 1, 3, or 5 miles",
              "Positive-price Search observations + certified relationship + frozen coordinates",
            ...
            [
              ...
              "Comparable benchmark stores ÷ distinct observed-distribution benchmark stores for an in-scope benchmark product",
              "Distinct positive-price benchmark-store grain",
              "Multiple positive-price products at one store still count the store once",
            ],
            [
            ...
            [
              ...
              "Certified relationship + positive-price local Search evidence on both sides",
            ...
            [
              ...
              "Product with observed store distribution and no governed eligible equivalent in the corresponding observed footprint",
              "Positive-price assortment evidence + relationship ledger + geography",
      ...
      blocks: [
        {
          ...
          items: [
            {
              term: "Observed store distribution",
              definition:
                "The count of distinct stores where the exact retailer product ID appeared in a store-level Search result with a listed price greater than $0. Each store counts once. It is not inventory or in-stock status and is never extrapolated to unobserved stores.",
            },
            {
            ...
            {
              term: "Observed service-area presence",
              definition:
                "The count of distinct service-area Search contexts where the exact retailer product ID appeared with a listed price greater than $0. It is always reported separately and never presented as stores sold.",
            },
            {
              term: "Observed distribution coverage",
              definition:
                "Distinct positive-price store-level Search locations divided by the searched store scope. The numerator is not projected beyond observed stores. Service-area coverage uses its own separately labeled population.",
            },
            {
              term: "Sponsored",
              definition:
                "Search is_sponsored=true for the product-location evidence. It is not a generalized promotion label. Sponsorship does not exclude an otherwise valid positive-price store Search result from observed distribution.",
            },
            {
              term: "Search-listed package price",
              definition:
                "The positive price returned for the offered package by a store- or service-area-context Search query.",
            },
            {
              term: "Unit price",
              definition:
            ...
            },
            {
              term: "Coverage",
              definition:
                "The portion of the intended comparison population with sufficient source, match, geography, freshness, and positive-price evidence to score. Semantic, observed-distribution, and price coverage are separate.",
            },
            {
              term: "Comparable store coverage",
              definition:
                "Distinct benchmark stores with at least one valid local competitor product comparison divided by distinct observed-distribution benchmark stores for an in-scope benchmark product. Multiple products at one store count once. Physical competitors report distinct contributing stores; service-area competitors report distinct delivery ZIPs.",
            },
            {
              term: "Whitespace / gap",
              definition:
        ...
        },
        {
          ...
          items: [
            ...
            },
            {
              ...
              definition:
                "A certified eligible relationship with admissible positive Search prices under the selected physical-store radius. Store distribution requires the exact retailer product in a store-level Search result with price greater than zero; stock and sponsorship fields do not determine distribution. Amazon Same Day remains explicitly labeled same-ZIP service-area evidence and is never counted as stores.",
            },
            {
      ...
      blocks: [
        {
          kind: "list",
          items: [
            "Corrected report contracts and category reprocessing use distinct positive-price store-level Search results as observed distribution, keep service-area presence separate, and do not expose store-level inventory status. A prior report is not current until its immutable reprocessed successor passes the release gate.",
            ...
            "Customer login, route-boundary auth, and role-based app/customer access are paused in production restore mode; /customer and customer-report routes redirect back to the legacy app, and only legacy administrator password sessions gate Administration. Durable account/RBAC tables remain dormant and future-facing.",
    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          kind: "callout",
          tone: "attention",
          title: "Current distribution authority",
          text: "Change-order entries preserve the terminology used when they were recorded. The owner's later September 8 correction supersedes the earlier same-day availability interpretation: observed store distribution is the distinct store footprint where an exact product appears in store-level Search with price greater than $0. It is not an inventory claim, does not use stock status, keeps service areas separate, and is never extrapolated.",
        },
        {
          kind: "table",
          ...
          rows: [
            ...
            [
              ...
              "The web app now runs in legacy restore mode: the Next.js proxy performs no customer or administrator authentication validation, routeAccessDecision classifies routes as public, the proxy hard-redirects dormant customer pages to legacy app destinations, /api/auth/login, /api/auth/callback, and /api/auth/logout redirect to the legacy root, /api/auth/me returns private no-store JSON 401, /api/customer/reports routes return a private no-store disabled response without proxying, and the WorkOS webhook route acknowledges with 204 without forwarding. Administration authorization is restored to the existing legacy password session through /api/admin/session and verifyAdminAccess; WorkOS-backed customer principals, route-cache cookies, and customer account-menu UI are not active. The root route renders the legacy operational home/dashboard, normal navigation is restored, simplified navigation is disabled, and /analyses/[analysisId] renders the legacy Blueprint Competitive Intelligence workspace instead of the newer canonical product report workspace. Proximity remains active and keeps its current map/reporting implementation, with client-side 401s surfaced as errors rather than redirect loops. This is a code-level rollback only: it does not drop account/RBAC database tables, remove Railway WorkOS secrets, delete WorkOS configuration, mutate raw evidence, change location rows, retailer eligibility, observed product distribution, matching decisions, report calculations, source-provider calls, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-15",
            ...
            [
              "2026-09-14",
              "Local verification complete",
              "Customer route quoted-cookie guard added.",
              "The protected route boundary now normalizes customer-auth cookie values from the framework parser or raw Cookie header and strips surrounding quotes before route-cache comparison or session-validation decisions. This addresses the observed post-callback pattern where /api/v1/me validated the sealed CPGHero customer session but the browser was still returned to /api/auth/login for /customer. This changes customer-route cookie recognition only; it does not change WorkOS credentials, user provisioning, canary allowlists, roles, entitlements, report calculations, proximity metrics, collection requests, source-provider calls, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-14",
              "Local verification complete",
              "Customer auth session-readiness guard added.",
              "Successful WorkOS/AuthKit callbacks now set production customer-session cookies with callback-compatible SameSite=None; Secure attributes and the private CPGHero completion page validates /api/auth/me before navigating into a protected app route. If the browser cannot read the new customer session, the page stops on a controlled CPGHero recovery state instead of automatically re-entering hosted login and triggering identity-provider rate limits. This changes customer-login callback/session handoff only; it does not change WorkOS credentials, user provisioning, canary allowlists, roles, entitlements, report calculations, proximity metrics, collection requests, source-provider calls, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-14",
              "Local verification complete",
              "Customer auth callback completion page added.",
              "Successful WorkOS/AuthKit callbacks now set the CPGHero customer-session cookie on a private first-party CPGHero completion page before navigating to the protected return path. This avoids relying on an immediate external-to-internal server-side redirect chain after Google social login, while preserving the temporary flow-cookie validation, canary allowlist, customer-session cookie, and downstream CPGHero account/workspace authorization. This changes customer-login callback delivery only; it does not change WorkOS credentials, user provisioning, roles, entitlements, report calculations, proximity metrics, collection requests, source-provider calls, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-14",
              "Local verification complete",
              "Public landing page and customer login recovery added.",
              "The root route now renders a public CPGHero landing page instead of immediately forcing customer login or exposing the internal operational dashboard shell. In-app brand and Home navigation now target the protected customer workspace. Customer AuthKit flow cookies use production callback-compatible attributes, and callbacks that arrive without the temporary login-flow cookie restart customer login instead of stranding the user on a terminal controlled-rollout 401 page. Protected app, analytics, report, collection, administrator, and same-origin API routes remain behind the route-auth boundary and downstream API authorization. This changes public entry and login recovery behavior only; it does not change WorkOS credentials, customer provisioning, roles, entitlements, report calculations, proximity metrics, collection requests, source-provider calls, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-14",
              "Local verification pending",
              "Customer route-session validation cache added.",
              "The Next.js customer route boundary now stores a short-lived CPGHero-signed validation cache cookie after a successful customer session validation. The cache is bound to the current customer session cookie, expires quickly, and is used only to avoid repeated same-origin customer session validation during rapid protected page/API navigation bursts. Missing, malformed, expired, tampered, or cross-session cache values fail closed to the existing validation path. Protected routes remain gated before page shells render or app API proxies run, and API handlers remain the authoritative enforcement layer for permissions, entitlements, account/workspace scope, report grants, and tenant-specific data access. This changes only route-boundary validation frequency and does not change WorkOS credentials, customer provisioning, roles, entitlements, report calculations, proximity metrics, collection requests, source-provider calls, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-14",
              "Local verification pending",
            ...
            [
              ...
              "WorkOS Staging now has CPGHero application redirect URIs, application homepage, login-initiation URI, and sign-out URI configured for the Railway app host and local callback. Railway production web and API services now store WorkOS client ID, API key, redirect URI, and cookie-password variables while CPGHERO_CUSTOMER_AUTH_PROVIDER remains disabled. The API adds /api/v1/me plus an explicit non-production-only customer-principal test harness that validates CPGHero role and entitlement keys and rejects production test headers. This is a safe identity seam only; it does not enable production customer login, create customer signup or invitation screens, create a webhook secret, add SSO, add SCIM, issue Live API keys, enforce row-level customer route scope, change account authorization, change reports, change proximity metrics, make paid source calls, make PDP calls, or make AI calls.",
            ],
            [
              "2026-09-13",
            ...
            [
              "2026-09-12",
              "Local verification passed; production deployment pending",
              "Proximity caps the competitor white-space drawer preview for responsiveness.",
              "The competitor white-space state drawer now renders only the first 100 matching rows in the browser while keeping the complete detail set available through CSV and JSON downloads. This protects browser and accessibility-tree performance for dense competitors such as CVS without changing the underlying API response, row-level evidence, state counts, metric denominators, location rows, coordinates, retailer eligibility, nearest-location Haversine math, product Search evidence, observed product distribution, matching, report calculations, seller rules, provider collection, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-12",
              "Local verification passed; production deployment pending",
              "Proximity adds competitor-side white-space state intelligence and fixes insight-card readability.",
              "The Proximity API now returns competitor-to-nearest-Walmart pair rows and competitor-state summaries so the page can identify where the selected competitor has mappable locations without a Walmart inside the selected radius. The fourth KPI now reports competitor sites beyond the radius from Walmart instead of the older represented-nearest-competitor count, and the state insight card now ranks competitor white-space by state with an All states drawer, reset path, capped in-browser row preview, and full CSV/JSON downloads. Radius coverage cards were restyled to avoid value collisions, and metric-style info buttons now sit in the top-right of their cards. This is a Proximity API, metric-denominator, state-drilldown, readability, and UX enhancement only; it does not change location rows, coordinates, retailer eligibility, nearest-location Haversine math, product Search evidence, observed product distribution, matching, report calculations, seller rules, provider collection, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-12",
              "Local verification passed; production deployment pending",
              "Proximity adds backend insight summaries and a reference-quality map workspace pass.",
              "The Proximity API now returns reusable distance-profile, state-coverage, and competitor-network summary objects in addition to the pair rows and fast map clusters. The web page moves radius coverage into a top insight deck with largest white-space states, highest-overlap competitor sites sorted by within-radius Walmart coverage, and median/P75/P90/max nearest-distance context; selected competitor networks now render as a dedicated map-side panel with coverage, selected-pair, median, farthest-distance, recenter, details, and table actions. The map legend is moved out of the top toolbar clutter and the MapLibre controls are positioned to avoid selected-network overlap. This is a Proximity API, UI, information hierarchy, source-definition, and performance-supporting enhancement only; it does not change location rows, coordinates, retailer eligibility, nearest-location pairing, Haversine distance math, product Search evidence, observed product distribution, matching, report calculations, seller rules, provider collection, PDP calls, AI calls, PDFs, or historical artifacts.",
            ],
            [
              "2026-09-12",
              "Local verification passed; production deployment pending",
            ...
            [
              ...
              "Production smoke testing found that the first report-level state filter implementation could return 503 for a large full-report request. The optimized read path scans only raw Search fields needed for state membership, applies the positive package-price and store-location rules, preserves latest product-location selection, and keeps seller-policy retractions. Production verified the full Milk report state coverage request for 436 products returned 52 state options in about 6 seconds after deployment. This is a read-model performance and reliability change only; it does not call providers, PDP, or AI, does not change source evidence, matching-v2 certification, price calculations, seller rules, distribution definitions, report replay, PDF export, or historical artifacts.",
            ],
            [
              "2026-09-10",
              "Merged, deployed, and superseded by the optimized state-index follow-up",
            ...
            [
              ...
              "Canonical report store-footprint labels now separate business footprint share from searched-row audit share.",
              "Executive Summary, Product Wins & Losses, Distribution & Assortment, Price Architecture, exact-product maps, and store-evidence drawers show product distribution against the retailer's report footprint denominator: the largest observed positive-price store footprint for that retailer in the current canonical report dataset. This denominator is not total chain stores, inventory, or a sampled returned-row count. Drawer JSON includes report_footprint_store_count, report_footprint_share, searched_row_share, and the retained searched-row metadata, while CSV and Excel-compatible downloads continue to expose the returned store rows. Distribution & Assortment keeps every governed Walmart product footprint visible and treats broad distribution as a metric, not a hidden filter. This is a presentation, UX, and export-metadata change only; it does not change source data, matching-v2 certification, report calculations, price normalization, seller rules, distribution definitions, provider collection, PDP calls, AI calls, report replay, PDF export, or historical artifacts.",
            ],
            [
              "2026-09-10",
              "Merged, CI-passed, and production-verified",
            ...
            [
              ...
              "Canonical report executive lists and location drilldowns were expanded so simplification does not remove product-level evidence.",
              "Executive Summary now renders comprehensive Walmart-loss and Walmart-win action lists instead of a top-N subset. Executive and Price Architecture rows show Walmart-vs-competitor positive-price store footprint counts and searched-store percentages when available. Store evidence drawers load full price-monitoring map evidence for exact product IDs, calculate sold-in-store share as observed distribution stores divided by observed distribution stores plus searched-not-observed store locations, and provide CSV, Excel-compatible, and JSON downloads that include both the numerator rows and the searched denominator rows. Service-area presence remains separate from store counts. This is a UI/report drill-down change only; it does not change source data, matching-v2 certification, report calculations, price normalization, seller rules, distribution definitions, provider collection, PDP calls, AI calls, report replay, PDF export, or historical artifacts.",
            ],
            [
              "2026-09-10",
            ...
            [
              ...
              "Retained-evidence reprocessing now preserves the owner-defined positive-price Search footprint through classified-row restoration and distinguishes exhaustive, explicitly limited retailer evidence from missing retailer evidence.",
              "Nonpositive optional regular and discounted price sentinels restore as missing without changing package price. A retailer with zero certified comparable relationships is nonblocking only when its checksum-governed certification is exhaustive, complete, fully reconciled, has zero pending review, and emits explicit no-governed-relationship scorecards. Under the same proof, a retailer with certified comparable identity but no positive-price observations satisfying the selected geography and comparison basis may emit only an explicit no-admissible-observations limitation with unavailable—not zero—price outcomes. All unaccounted, incomplete, mixed-state, or zero-valued evidence still fails closed. Banana, Milk, Ground Beef, and Strawberries corrected generations are active. Eggs and Vitamins remain quarantined until the correction is deployed, their jobs are explicitly retried, and activation is verified; no provider, AI, PDP, collection, or paid call is required.",
            ],
            [
              "2026-09-08",
            ...
            [
              "2026-09-08",
              "Superseded by the owner-defined distribution correction recorded immediately above",
              "The interim stock-status interpretation in this entry is not current authority.",
              "Price Observation 1.3.0, Price Monitoring View 1.5.0, Price Monitoring Map 1.3.0, and Price Architecture Matrix 1.3.0 use the explicit positive-price Search distribution contract. A distinct physical store counts once per exact retailer product when that product appears in store-level Search with price greater than zero. Service-area presence is stored separately and never counted as stores. Stock status and sponsorship do not determine distribution, and no retailer location master is used to extrapolate to unobserved stores. Retained immutable evidence can be rebuilt through the governed publication path without a provider or AI call.",
            ],
            [
              "2026-08-30",
