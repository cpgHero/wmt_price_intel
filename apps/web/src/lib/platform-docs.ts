      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "Next.js route-boundary protection now validates the expected CPGHero customer or administrator session through same-origin auth/session endpoints before protected pages or protected same-origin app/admin API proxies run. The root route is a public CPGHero landing page with sign-in entry points; protected app workspace, analytics, customer, report, collection, and administrator pages remain behind route-boundary checks. Anonymous or invalid top-level customer navigations redirect from protected app workspace and analytics pages to customer login, anonymous or invalid administrator sessions redirect from administrator pages to the dedicated administrator login route, and protected same-origin app/admin API reads return private no-store JSON 401 responses. Customer page/API shell validation uses a short-lived CPGHero-signed route-validation cache cookie bound to the current customer session cookie so rapid app requests do not repeatedly hit the customer identity/session validator. If an AuthKit callback reaches CPGHero without the temporary login-flow cookie, the callback restarts customer login instead of presenting a terminal 401 page. Successful callback responses preserve every upstream Set-Cookie mutation independently, set the sealed CPGHero customer session on a private first-party CPGHero completion page, clear the temporary login-flow cookie, and then navigate to the protected app return path after the browser has a stable CPGHero response on which to commit the session. Background RSC, prefetch, and other non-HTML-document requests fail closed with private no-store 401 responses and cannot launch a hosted login session from either the protected-route boundary or /api/auth/login. API route handlers remain responsible for authoritative permission, entitlement, account, workspace, report-grant, and admin-operation enforcement.",
        ...
        },
        {
          ...
          text: "The durable account/RBAC/entitlement foundation exists, WorkOS AuthKit is externally configured, migration 0055 adds external identity mappings, and migration 0056 adds owner-only invitation/provisioning state plus signed identity webhook audit. The API can resolve either a validated non-production test principal or a live WorkOS sealed session into the CPGHero principal model, and WorkOS callbacks fail closed to a canary allowlist whenever the provider is enabled. The public root landing page is reachable without login, while Next.js validates the expected customer or administrator session before protected app workspace, analytics, customer, and administrator page shells load, and protected same-origin app/admin API routes return JSON 401 before reaching legacy global data proxies when the session is absent or invalid. Customer route-shell validation is cached for a short CPGHero-signed window bound to the current session cookie to protect availability during navigation bursts; route-cache acceptance is not an authorization decision and does not replace downstream API authorization. The customer-auth proxy preserves separate callback cookies so successful AuthKit callbacks can establish the HttpOnly CPGHero session, and the callback completion page now verifies /api/auth/me before navigating into protected app routes so a failed browser-cookie handoff cannot create an automatic hosted-login loop. Successful /api/auth/me checks also prime the same short-lived route cache before the browser enters the protected app shell. The route boundary and /api/auth/me route normalize quoted sealed cookie values from either the framework parser or raw Cookie header before deciding whether to validate, cache, or redirect. Background RSC/prefetch/non-document traffic cannot create parallel hosted-login sessions. This route-boundary guard is not a substitute for tenant-scoped API authorization: customer-facing signup/invite screens, customer API keys, account-administration screens, and cross-account enforcement across legacy report, collection, and proximity data routes remain future work.",
        },
      ],
    },
    {
