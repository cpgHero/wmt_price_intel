      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            "The canonical product report uses five distinct app tabs: Executive Summary for complete win/loss action lists, Product Wins & Losses for the image-first relationship action board, Distribution & Assortment for deduplicated Walmart product footprints plus exact-product location evidence, Price Architecture for table-based reporting price ladders, and Evidence & QA for guardrails and exclusions. Executive and price tables must not truncate governed win/loss lists. They show Walmart and competitor positive-price store footprints with searched-store share where the denominator is available. Distribution & Assortment loads source-backed product location maps and store-evidence drawers through the price-monitoring product view; drawers expose store-list detail with CSV, Excel-compatible, and JSON downloads. Drawer exports include observed distribution store rows and searched-not-observed store rows so the footprint numerator, searched-store denominator, and store-share percentage are auditable. If map evidence is unavailable, the report must say so rather than drawing inferred geography.",
