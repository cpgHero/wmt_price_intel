        ...
        },
        {
          ...
          rows: [
            [
              ...
              "The three primary immutable runs estimate 77,663 credits ($155.326). After four successful Walmart Banana preflight responses and one nonbillable provider HTTP 500 failed that retailer gate, a Walmart-only recovery reused the frozen geography with a 5,000-credit ($10) ceiling instead of rewriting history. Combined per-run ceilings are now $180, leaving $20 uncommitted. Search starts are shared-limit governed at three per second and 180 per minute per retailer, with three worker replicas and unique owners. A completed request refills its slot without waiting for a slow batch peer, claims round-robin collection-run/retailer lanes, and applies a per-replica retailer concurrency ceiling so a slow endpoint cannot consume every slot needed by other retailers. Candidate rows are materialized under SKIP LOCKED and the final update rechecks pending or expired-lease eligibility, preventing a concurrent stale candidate from receiving a second live lease. The expanded HTTP pool changes concurrency only; the Postgres limiter still owns request starts. Durable heartbeats, retailer gates, shared cooldowns, retry limits, cancellation, budgets, and immutable raw evidence remain intact. No PDP or OpenAI spend is authorized by this phase; final credit and artifact reconciliation remains pending terminal runs.",
            ],
            [
              "2026-08-29",
